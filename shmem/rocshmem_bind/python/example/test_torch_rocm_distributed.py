...
@torch.no_grad()
def ag_gemm_torch(input: torch.Tensor, weight: torch.Tensor, warmup: int,
                  iters: int):
    ...
    full_input = torch.zeros((M, input.size(1)),
                             dtype=input.dtype,
    ...
    for i in range(total_iters):
        ...
        torch.distributed.all_gather_into_tensor(full_input,
                                                 input,
                                                 group=TP_GROUP)
...
def test_ipc_handle():
    ...
    # test ipc handle
    input_buffer = torch.zeros((M, K),
                               dtype=dtype,
                               device=device,
                               requires_grad=False)
    input_buffer_offset = input_buffer.storage_offset()
    shm_handle = input_buffer._typed_storage()._share_cuda_()[
        1]  # cudaIpcMemHandle_t
    ...
    shm_handle_ts_cuda = torch.ByteTensor(
        torch.ByteStorage._from_buffer(shm_handle)).cuda()
    shm_handles = [
        torch.empty_like(shm_handle_ts_cuda) for _ in range(WORLD_SIZE)
    ]
    torch.distributed.all_gather(shm_handles,
                                 shm_handle_ts_cuda,
                                 group=TP_GROUP)
    ...
    signal_handle_ts_cuda = torch.ByteTensor(
        torch.ByteStorage._from_buffer(signal_handle)).cuda()
    signal_handles = [
        torch.empty_like(signal_handle_ts_cuda) for _ in range(WORLD_SIZE)
    ]
    torch.distributed.all_gather(signal_handles,
                                 signal_handle_ts_cuda,
                                 group=TP_GROUP)
    ...
    torch.distributed.all_gather_object(signal_offset_list,
                                        signal_total_offset,
                                        group=TP_GROUP)
    print(f"currank={TP_GROUP.rank()}, signal_handles:{signal_handles}")
    print(
        f"currank={TP_GROUP.rank()}, signal_offset_list:{signal_offset_list}")

    shm_buffers = pyrocshmem.rocshmem_get_tensors_from_ipchandle(
        LOCAL_RANK, WORLD_SIZE, shm_handles, offset_list, input_buffer.shape,
        dtype)
    shm_buffers.insert(LOCAL_RANK, input_buffer)

    ipcshm_buffers = pyrocshmem.rocshmem_create_tensor_list_intra_node(
    ...
    torch.distributed.barrier()
    print(
        f"after currank={TP_GROUP.rank()}, local_tensor:{ipcshm_buffers[LOCAL_RANK % WORLD_SIZE]}"
    )
...
if __name__ == '__main__':
    ...
    input = (torch.rand(
        (local_M, K), dtype=dtype).cuda() / 100 * ((TP_GROUP.rank() + 1)**2))
    weight = (torch.rand(
        (local_N, K), dtype=dtype).cuda() / 100 * ((TP_GROUP.rank() + 1)**2))
