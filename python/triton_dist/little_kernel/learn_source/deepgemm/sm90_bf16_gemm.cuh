...
template <uint32_t SHAPE_M, uint32_t SHAPE_N, uint32_t SHAPE_K,
          uint32_t kNumGroups, uint32_t BLOCK_M, uint32_t BLOCK_N,
          uint32_t BLOCK_K, uint32_t kSwizzleDMode, uint32_t kNumStages,
          uint32_t kNumLastStages, uint32_t kNumTMAThreads,
          uint32_t kNumMathThreads, uint32_t kNumTMAMulticast,
          bool kIsTMAMulticastOnA, uint32_t kNumSMs, GemmType kGemmType,
          typename cd_dtype_t>
__global__
__launch_bounds__(kNumTMAThreads + kNumMathThreads, 1) void sm90_bf16_gemm_impl(
  ...
  static constexpr uint32_t SMEM_D_SIZE =
      BLOCK_M * BLOCK_N * sizeof(cd_dtype_t);
  static constexpr uint32_t SMEM_A_SIZE_PER_STAGE =
      BLOCK_M * BLOCK_K * sizeof(__nv_bfloat16);
...
#pragma unroll
  ...
  auto scheduler = Scheduler<kGemmType, BLOCK_M, BLOCK_N, kNumGroups,
      ...
      while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
        launch_k_iterations([&](uint32_t k_iter, auto divisible_type) {
          constexpr bool kHasDivisibleStages =
              cute::is_same_v<decltype(divisible_type), DivisibleK>;
...
#pragma unroll
          for (uint32_t s = 0; s < kNumInnerStages; ++s) {
            ...
            constexpr bool kWithGroupOffsetA =
            ...
            tma_copy(&tensor_map_a, reinterpret_cast<uint64_t *>(&full_barrier),
                     smem_a[s], k_idx,
                     scheduler.get_global_idx<kWithGroupOffsetA>(
                         shape_m, BLOCK_M, m_block_idx),
                     num_tma_multicast_a);
            tma_copy(&tensor_map_b, reinterpret_cast<uint64_t *>(&full_barrier),
                     smem_b[s], k_idx,
                     scheduler.get_global_idx<true>(shape_n, BLOCK_N,
          ...
          for (auto i = 0; i < WGMMA::kNumAccum / 4; ++i) {
            ...
            if constexpr (kSwizzleDMode > 0) {
              ...
              auto row = kHasShortcut ? (in_atom_offset / 8 + lane_idx)
                                      : (bank_group_index / 8);
              auto col =
                  kHasShortcut ? (in_atom_offset) : (bank_group_index % 8);
...
#pragma unroll
        for (uint32_t local_idx = 0; local_idx < BLOCK_M / WAVE_BLOCK_M;
          ...
          auto smem_d_0 = reinterpret_cast<float2 *>(
              smem_d +
              (m_offset + warp_idx * WGMMA_M_PER_WARP + lane_idx / 4 + 0) *
                  BLOCK_N +
              (lane_idx % 4) * 2);
          auto smem_d_1 = reinterpret_cast<float2 *>(
              smem_d +
              (m_offset + warp_idx * WGMMA_M_PER_WARP + lane_idx / 4 + 8) *
                  BLOCK_N +
              (lane_idx % 4) * 2);
#pragma unroll
          for (auto i = 0; i < WGMMA::kNumAccum / 4; ++i) {
            st_shared(smem_d_0 + i * 4, make_float2(shifted_accum[i * 4 + 0],
                                                    shifted_accum[i * 4 + 1]));
            st_shared(smem_d_1 + i * 4, make_float2(shifted_accum[i * 4 + 2],
                                                    shifted_accum[i * 4 + 3]));
          }
        }
      }
      ...
      if (threadIdx.x < BLOCK_N / TMA_D_BLOCK_N) {
        ...
        cute::SM90_TMA_STORE_2D::copy(
            ...
            scheduler.get_global_idx<kWithGroupOffsetD>(shape_m, BLOCK_M,
                                                        m_block_idx));
