...
def extract(input: tl.tensor, indices: List, builder: ir.builder) -> tl.tensor:
    dst_indices = []
    for idx in indices:
        if isinstance(idx, tl.tensor):
            dst_indices.append(idx.handle)
        elif isinstance(idx, tl.constexpr):
...
def insert(input: tl.tensor, scalar, indices, builder: ir.builder) -> tl.tensor:
    if isinstance(indices, (tl.tensor, tl.constexpr)):
        indices = [indices]
    dst_indices = []
    for idx in indices:
        if isinstance(idx, tl.tensor):
            ...
            raise ValueError(f"unsupported tensor index: {idx}")
    return tl.tensor(builder.create_insert(scalar.handle, input.handle, dst_indices), input.type)
